...
def IVSO(Xcal, ycal, A_max=10, fold=10, method='autoscaling', num_bms=5000):
    ...
    for iter in range(times):
        ...
        # 使用 joblib 并行计算 B 矩阵的每一行
        B_rows = Parallel(n_jobs=-1, verbose=0)(
            delayed(compute_regression_coefficients)(i, V_binary_matrix, Xcal, ycal, A_max, fold, method)
        ...
        elapsed_time = time() - round_time
        print(f'第 {iter + 1} 次 IVSO 迭代已完成，耗时 {elapsed_time:.2f} s')
...
def compute_regression_coefficients(i, V_binary_matrix, Xcal, ycal, A_max, fold, method):
    ...
    B_row = np.zeros_like(temp, dtype=float)
    for j in range(int(np.sum(V_binary_matrix[i, :]))):
        B_row[selected_indices[j]] = abs(PLS['coef_origin'][j, A_origin - 1])

    if np.max(B_row) != 0:
        B_row /= np.max(B_row)  # 归一化回归系数
...
mat_data = loadmat(r'\corn moisture.mat')
Xcal=mat_data['Xcal']
ycal=mat_data['ycal']
...
运行结果：
开始第 1 次迭代...
第 1 次 IVSO 迭代已完成，耗时 0.92 min
开始第 2 次迭代...
第 2 次 IVSO 迭代已完成，耗时 0.40 min
开始第 3 次迭代...
第 3 次 IVSO 迭代已完成，耗时 0.35 min
开始第 4 次迭代...
第 4 次 IVSO 迭代已完成，耗时 0.33 min
开始第 5 次迭代...
第 5 次 IVSO 迭代已完成，耗时 0.32 min
开始第 6 次迭代...
第 6 次 IVSO 迭代已完成，耗时 0.18 min
开始第 7 次迭代...
第 7 次 IVSO 迭代已完成，耗时 0.13 min
开始第 8 次迭代...
第 8 次 IVSO 迭代已完成，耗时 0.12 min
开始第 9 次迭代...
第 9 次 IVSO 迭代已完成，耗时 0.12 min
开始第 10 次迭代...
第 10 次 IVSO 迭代已完成，耗时 0.12 min
开始第 11 次迭代...
第 11 次 IVSO 迭代已完成，耗时 0.12 min
IVSO 的迭代已全部完成，总耗时 3.13 min
RMSEP: 0.000357182422486004, RMSEF: 0.00025819425910679093
